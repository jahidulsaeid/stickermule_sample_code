...
import { useGetProductsQuery, useOnSaleQuery } from '@/redux/features/products/productApi';
...
const OnSellingProducts: React.FC<Props> = () => {
  const [disablePrev, setDisablePrev] = useState(true);
  const [disableNext, setDisableNext] = useState(false);
  SwiperCore.use([Autoplay]);
  // const { data: products, isLoading } = useGetProductsQuery();
  const { data: products, isLoading } = useOnSaleQuery();
    ...
    <div className="container mx-auto px-3 md:px-0">
      <div className="flex items-center justify-between py-10">
        ...
        <div className="slider_icon flex gap-2">
          <button className="onslider_prev">
            <svg
              className=""
              width={40}
              height={40}
          ...
          <button className="onslider_next">
            <svg
              className=""
              width={40}
              height={40}
      ...
      <div className="container">
        <Swiper
          ...
          spaceBetween={20}
          navigation={{
            nextEl: '.onslider_next',
            prevEl: '.onslider_prev',
          }}
          modules={[Navigation]}
          autoplay={{
            delay: 5000,
