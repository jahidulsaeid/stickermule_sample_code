...
const ListMenu = ({ dept, data, hasSubMenu, menuIndex }: any) => {
  return (
    <li className="relative">
      <Link
        href={data.path}
        className="flex items-center gap-3 py-2 ps-5 xl:ps-7 pe-3 xl:pe-3.5 hover:text-heading hover:bg-gray-300 text-xs"
      >
        {data.icon && (
          <Image src={data.icon} alt={data.label} width={30} height={30} className='w-6 h-6' />
        )}
        {data.label}
        {data.subMenu && (
